...
class Chassis(Module):
    ...
    def __init__(self, robot: robomaster.robot.Robot, node: 'RoboMasterROS') -> None:
        ...
        self.twist_to_wheel_speeds: bool = node.declare_parameter(
            "chassis.twist_to_wheel_speeds", True, descriptor=desc).value
        desc = rcl_interfaces.msg.ParameterDescriptor(
            description=(
    ...
    def has_received_twist(self, msg: geometry_msgs.msg.Twist) -> None:
        if self.twist_to_wheel_speeds:
            if MECANUM_WHEEL == False:
                ...
                pomer = 100
                self.api.drive_wheels(
                    w1=(msg.linear.x + msg.linear.y + msg.angular.z*(dx+dy)/wheel_r/180*math.pi*5)*pomer, w2=(msg.linear.x - msg.linear.y - msg.angular.z*(dx+dy)/wheel_r/180*math.pi*5)*pomer,
                    w3=(msg.linear.x + msg.linear.y - msg.angular.z*(dx+dy)/wheel_r/180*math.pi*5)*pomer, w4=(msg.linear.x - msg.linear.y + msg.angular.z*(dx+dy)/wheel_r/180*math.pi*5)*pomer,
                    timeout=self.timeout)
        else:
